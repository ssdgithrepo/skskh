...
function list(t,p){let u=auth(t);if(p.entity==='dashboard')return{ok:true,rows:[{products:read('Products').length,sales:read('Sales').length,purchases:read('Purchases').length,customers:read('Customers').length}]};if(!SCHEMAS[p.entity])throw Error('Invalid entity');return{ok:true,rows:read(p.entity).filter(r=>!u.branchId||!r.branchId||String(r.branchId)===String(u.branchId))}}
function create(t,p){let u=auth(t),n=p.entity;if(!SCHEMAS[n])throw Error('Invalid entity');authorize(u,n,'CREATE');let r={};SCHEMAS[n].forEach(k=>r[k]=p.data?.[k]??'');r.id=Utilities.getUuid();r.createdAt=new Date().toISOString();if(SCHEMAS[n].includes('createdBy'))r.createdBy=u.username;if(SCHEMAS[n].includes('branchId')&&!r.branchId)r.branchId=u.branchId;append(n,r);audit(u,'CREATE',n,r.id,r);return{ok:true,row:r}}
function update(t,p){let u=auth(t),n=p.entity;if(!SCHEMAS[n])throw Error('Invalid entity');authorize(u,n,'UPDATE');if(!p.id)throw Error('Record id is required');let s=SpreadsheetApp.openById(SS_ID).getSheetByName(n);let h=s.getRange(1,1,1,s.getLastColumn()).getValues()[0];let vals=s.getDataRange().getValues();let idx=vals.findIndex((r,i)=>i>0&&String(r[h.indexOf('id')])===String(p.id));if(idx<1)throw Error('Record not found');let old=Object.fromEntries(h.map((k,i)=>[k,vals[idx][i]]));let merged={...old,...(p.data||{}),id:p.id,updatedAt:new Date().toISOString()};if(h.includes('createdAt'))merged.createdAt=old.createdAt;if(h.includes('createdBy'))merged.createdBy=old.createdBy;s.getRange(idx+1,1,1,h.length).setValues([h.map(k=>merged[k]??'')]);audit(u,'UPDATE',n,p.id,merged);return{ok:true,row:merged}}
function deleteRecord(t,p){let u=auth(t),n=p.entity;if(!SCHEMAS[n])throw Error('Invalid entity');authorize(u,n,'DELETE');if(!p.id)throw Error('Record id is required');let s=SpreadsheetApp.openById(SS_ID),sh=s.getSheetByName(n),h=sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0],idc=h.indexOf('id'),vals=sh.getDataRange().getValues(),idx=vals.findIndex((r,i)=>i>0&&String(r[idc])===String(p.id));if(idx<1)throw Error('Record not found');let old=Object.fromEntries(h.map((k,i)=>[k,vals[idx][i]]));sh.deleteRow(idx+1);audit(u,'DELETE',n,p.id,old);return{ok:true}}

function transaction(t,p){
  let u=auth(t),n=p.entity,d=p.data||{};
